...
float3 SamplePbrShadowCascade(int cascadeIndex, float3 worldPosition)
{
    ...
    switch (cascadeIndex)
    {
    case 0: result = CalcShadowColorPCFFilter(shadowMaps[0], shadowSampler, coord,
        shadowColor.rgb, shadowBias, pcfKernelSize); break;
    case 1: result = CalcShadowColorPCFFilter(shadowMaps[1], shadowSampler, coord,
        shadowColor.rgb, shadowBias, pcfKernelSize); break;
    case 2: result = CalcShadowColorPCFFilter(shadowMaps[2], shadowSampler, coord,
        shadowColor.rgb, shadowBias, pcfKernelSize); break;
    default: result = CalcShadowColorPCFFilter(shadowMaps[3], shadowSampler, coord,
        shadowColor.rgb, shadowBias, pcfKernelSize); break;
    }
	return result;
}
...
float4 main(VS_OUT pin) : SV_TARGET
{
    ...
    * baseColor;
    ...
    if (cascadeIndex >= 0)
    {
        shadow = SamplePbrShadowCascade(cascadeIndex, pin.position);
        if (cascadeIndex < 3)
        {
            float blend = CalcShadowCascadeBlend(cameraDistance, cascadeIndex, cascade_splits);
            shadow = lerp(shadow,
                SamplePbrShadowCascade(cascadeIndex + 1, pin.position), blend);
        }
    }
