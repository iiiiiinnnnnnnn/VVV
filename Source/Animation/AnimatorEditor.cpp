...
ed::PinId Animator::OutPin(int li, int si)
    {
		// NodeEditorのIDは種類ごとに範囲を分離する。レイヤーと状態番号を埋め込み、
		// UIから返されたIDだけで元のデータ位置を逆算できるようにする。
...
void Animator::HandleInteractions(Animator::AnimatorLayer& layer, int li)
    {
        if (ed::BeginCreate())
        {
			// 出力Pinと入力Pinから状態番号を復元し、同じ組み合わせの重複遷移は作らない。
			// AnyStateだけは通常状態と別配列へ格納する。
        ...
        if (ed::BeginDelete())
        {
            ...
            while (ed::QueryDeletedNode(&delNode))
            {
                if (ed::AcceptDeletedItem())
                {
                    ...
                    {
						// 状態を消すと後続インデックスが一つずれるため、参照する全遷移と
						// currentStateIndexを同じ規則で補正する。
...
void Animator::DrawStateDetail(Animator::AnimatorLayer& layer, int li)
    {
        ...
        if (!state.transitions.empty())
        {
            ...
            {
				// 配列順を遷移評価順として使う。並べ替え後にpriorityを振り直し、
				// 選択中の要素が同じ遷移を指し続けるようインデックスも追従させる。
