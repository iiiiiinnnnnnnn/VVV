...
public:
    ModelRender(Actor* owner, std::shared_ptr<Model> model, ModelShaderId shaderId = ModelShaderId::Basic);
    ...
    void SetModelShaderId(ModelShaderId id) { shaderId = id; }
    void AppendNode(const Model::Node* node) { appendNode = node; }

    Model* GetModel() const { return model.get(); }
	const ModelShaderId& GetModelShaderId() const { return shaderId; }
